...
expl_names <- c('Intercept', 'Human footprint', 'Forest loss', 'Net primary production', 'Habitat diversity', 'Mean temperature')
```
<br>
...
We calculated $\beta_{\text{eqv}}$ as the total diversity of a unit ($\gamma_{\text{eqv}}$) minus the average diversity of BBA points ($\alpha_{\text{eqv}}$) within a unit, and calculated $\beta_{\text{prop}}$ as the proportion of $\gamma_{\text{eqv}}$ explained by $\beta_{\text{eqv}}$. We then normalised $\beta_{\text{prop}}$ by taking the number of communities in each unit into account using equation 14 from De Bello et al. 2010:
    
$$\beta_{\text{norm-prop}} = \frac{\beta_{\text{prop}}} {1-\frac{1} {n}}$$

This normalised measure of $\beta_{\text{prop}}$ enabled us to compare the proportion of $\gamma_{\text{eqv}}$ explained by $\beta_{\text{eqv}}$ between units with differing numbers of bird count points. In addition, using a proportional measure of $\beta_{\text{eqv}}$ enabled us to directly compare the proportion explained by different facets of biodiversity (taxonomic, functional, and phylogenetic).
...
We calculated net primary production (NPP)
...
explanatory_variables <- units_merged_beta_subset@data[c('human_footprint', 
  'forest_loss', 'net_prim_prod', 'habitat_div', 'mean_temp')]
explanatory_summary_table <- as.data.frame(sapply(explanatory_variables, summary_fun))
colnames(explanatory_summary_table) <- expl_names[2:6]
...
tax_model_coef <- as.data.frame(tax_model_summary$coefficients)
...
tax_hfp_change <- round(tax_model_coef['Human footprint', 'Estimate'] * hfp_range, digits = 2)
tax_hfp_perc <- round((tax_hfp_change / tax_range) * 100, digits = 2)
...
Taxonomic beta diversity was impacted by human footprint index (HFI), but not forest loss. Increasing levels of HFI were associated with increasing levels of taxonomic beta diversity. Along the range of human footprint values (`r tabRef("explanatory_summary")`) taxonomic beta diversity increased by `r tax_hfp_change` units (`r tax_hfp_perc`%). Taxonomic beta diversity was also positively associated with habitat diversity, although the effect was not statistically significant. Along the range of habitat diversity values (`r tabRef("explanatory_summary")`), taxonomic beta diversity increased by `r tax_hdiv_change` units (`r tax_hdiv_perc`%).
...
Similar to taxonomic beta diversity, functional beta diversity had a statistically significant association with HFI. Along the gradient of HFI in our data (`r tabRef("explanatory_summary")`), functional beta diversity increased strongly (`r func_hfp_change` units, `r func_hfp_perc`%). Forest loss did not appear to influence functional beta diversity. In addition, net primary production (NPP) had a strong negative impact on functional beta diversity. Along the range of NPP values (`r tabRef("explanatory_summary")`) functional beta diversity decreased by `r abs(func_npp_change)` units (`r func_npp_perc`%). Similarly, mean annual temperature had a negative impact, decreasing functional beta diversity by `r abs(func_temp_change)` units (`r func_temp_perc`%).
...
phyl_model_coef <- as.data.frame(phyl_model_summary$Coef)
...
Phylogenetic beta diversity was not impacted by HFI or forest loss. As was the case with functional beta diversity, phylogenetic beta diversity was negatively associated with NPP and mean temperature, although the biological impacts much stronger. Along the range of NPP values (`r tabRef("explanatory_summary")`) phylogenetic beta diversity decreased by `r abs(phyl_npp_change)` units (-`r phyl_npp_perc`%). Along the range of mean annual temperature values phylogenetic beta diversity decreased by `r abs(phyl_temp_change)` units (-`r phyl_temp_perc`%).
<br>
...
* Habitat diversity was taken into account, so increased beta diversity due to HFI increasing habitat diversity is not the answer
* Bird counts were conducted from road sides, so bird communities were not in pristine areas
  
* Species richness is not a driver of beta-diversity in our data
* Beta-diversity indices were not positive functions of net primary productivity or ambient temperature.
* We rather found that beta-diversities were negatively related to NPP and mean temperature.
* **Why is this?**  

* Why was phylogenetic diversity not impacted by human influence?
<br>
...
psych::pairs.panels(units_merged_beta_subset@data[c('human_footprint', 
  'forest_loss', 'habitat_div', 'net_prim_prod', 'mean_temp')], 
...
colnames(vif_table) <- expl_names[2:6]
