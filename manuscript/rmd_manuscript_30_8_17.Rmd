...
expl_names_1 <- c('Intercept', 'Human footprint', 'Forest loss', 'Net primary production', 'Habitat diversity', 'Mean temperature')
expl_names_2 <- c('Intercept', 'Human footprint', 'Forest loss', 'Net primary production', 'Habitat diversity', 'Mean temperature', 'Number forested points')
```
<br>
...
colnames(explanatory_summary_table) <- expl_names_2[2:7]
...
tax_model_coef <- as.data.frame(tax_beta_model_summary$coefficients)
colnames(tax_model_coef)[4] <- 'p value'
rownames(tax_model_coef) <- expl_names_2
...
func_model_coef <- as.data.frame(func_beta_model_summary$coefficients)
colnames(func_model_coef)[4] <- 'p value'
rownames(func_model_coef) <- expl_names_2
...
phyl_model_coef <- as.data.frame(phyl_beta_model_summary$Coef)
colnames(phyl_model_coef)[4] <- 'p value'
rownames(phyl_model_coef) <- expl_names_2
...
* Habitat diversity was taken into account, so increased beta diversity due increased habitat diversity caused by increased human influence is not the answer
* Bird counts were conducted from road sides, so bird communities were not in pristine areas
  
* Why did forest loss not impact beta diversity?
...
colnames(vif_table) <- expl_names_2[2:7]
