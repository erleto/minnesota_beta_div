...
output:
  word_document:
    ...
    fig_width: 6
    fig_height: 6
    reference_doc: manuscript_style.docx
    pandoc_args: ["+smart"]
---
...
func_alpha_hfp_plot <- ggplot(units_merged_beta_subset@data, 
  aes(x = human_footprint, y = func_alpha_div)) + 
  geom_point(shape = 1) + 
  labs(x = '', y = 'Functional alpha diversity') + 
  mytheme

func_beta_hfp_plot <- ggplot(units_merged_beta_subset@data, 
  aes(x = human_footprint, y = func_beta_div)) + 
  geom_point(shape = 1) + 
  geom_smooth(method = lm, se = FALSE, fullrange = TRUE, col = 1) + 
  geom_abline(intercept = 10.4, slope = 0.0447, linetype = 'dashed', col = 'red') + 
  labs(x = 'Human Footprint Index', y = 'Functional beta diversity') + 
  mytheme

func_gamma_hfp_plot <- ggplot(units_merged_beta_subset@data, 
  aes(x = human_footprint, y = func_gamma_div)) + 
  geom_point(shape = 1) + 
  labs(x = '', y = 'Functional gamma diversity') + 
  mytheme

multiplot(func_alpha_hfp_plot, func_beta_hfp_plot, func_gamma_hfp_plot, cols = 3)
```
  
```{r, echo = FALSE, message = FALSE, warning = FALSE, fig.width = 12, fig.height = 4}
func_alpha_floss_plot <- ggplot(units_merged_beta_subset@data, 
  aes(x = forest_loss, y = func_alpha_div)) + 
  geom_point(shape = 1) + 
  geom_smooth(method = lm, se = FALSE, fullrange = TRUE, col = 1) + 
  geom_abline(intercept = 1.44, slope = -0.00344, linetype = 'dashed', col = 'red') + 
  labs(x = '', y = 'Functional alpha diversity') + 
  mytheme

func_beta_floss_plot <- ggplot(units_merged_beta_subset@data, 
  aes(x = forest_loss, y = func_beta_div)) + 
  geom_point(shape = 1) + 
  labs(x = 'Forest loss (%)', y = 'Functional beta diversity') + 
  mytheme

func_gamma_floss_plot <- ggplot(units_merged_beta_subset@data, 
  aes(x = forest_loss, y = func_gamma_div)) + 
  geom_point(shape = 1) + 
  geom_smooth(method = lm, se = FALSE, fullrange = TRUE, col = 1) + 
  geom_abline(intercept = 1.59, slope = -0.00286, linetype = 'dashed', col = 'red') + 
  labs(x = '', y = 'Functional gamma diversity') + 
  mytheme

multiplot(func_alpha_floss_plot, func_beta_floss_plot, func_gamma_floss_plot, cols = 3)
```
...
```{r, echo = FALSE, message = FALSE, warning = FALSE, fig.width = 12, fig.height = 4}
phyl_alpha_hfp_plot <- ggplot(units_merged_beta_subset@data, 
  aes(x = human_footprint, y = phyl_alpha_div)) + 
  geom_point(shape = 1) + 
  geom_smooth(method = lm, se = FALSE, fullrange = TRUE, col = 1) + 
  geom_abline(intercept = 1.6, slope = -0.00161, linetype = 'dashed', col = 'red') + 
  labs(x = '', y = 'Phylogenetic alpha diversity') + 
  mytheme

phyl_beta_hfp_plot <- ggplot(units_merged_beta_subset@data, 
  aes(x = human_footprint, y = phyl_beta_div)) + 
  geom_point(shape = 1) + 
  labs(x = 'Human Footprint Index', y = 'Phylogenetic beta diversity') + 
  mytheme

phyl_gamma_hfp_plot <- ggplot(units_merged_beta_subset@data, 
  aes(x = human_footprint, y = phyl_gamma_div)) + 
  geom_point(shape = 1) + 
  labs(x = '', y = 'Phylogenetic gamma diversity') + 
  mytheme

multiplot(phyl_alpha_hfp_plot, phyl_beta_hfp_plot, phyl_gamma_hfp_plot, cols = 3)
```
  
```{r, echo = FALSE, message = FALSE, warning = FALSE, fig.width = 12, fig.height = 4}
phyl_alpha_floss_plot <- ggplot(units_merged_beta_subset@data, 
  aes(x = forest_loss, y = phyl_alpha_div)) + 
  geom_point(shape = 1) + 
  geom_smooth(method = lm, se = FALSE, fullrange = TRUE, col = 1) + 
  geom_abline(intercept = 1.6, slope = -0.00577, linetype = 'dashed', col = 'red') + 
  labs(x = '', y = 'Phylogenetic alpha diversity') + 
  mytheme

phyl_beta_floss_plot <- ggplot(units_merged_beta_subset@data, 
  aes(x = forest_loss, y = phyl_beta_div)) + 
  geom_point(shape = 1) + 
  labs(x = 'Forest loss (%)', y = 'Phylogenetic beta diversity') + 
  mytheme

phyl_gamma_floss_plot <- ggplot(units_merged_beta_subset@data, 
  aes(x = forest_loss, y = phyl_gamma_div)) + 
  geom_point(shape = 1) + 
  geom_smooth(method = lm, se = FALSE, fullrange = TRUE, col = 1) + 
  geom_abline(intercept = 1.85 , slope = -0.00752, linetype = 'dashed', col = 'red') + 
  labs(x = '', y = 'Phylogenetic gamma diversity') + 
  mytheme

multiplot(phyl_alpha_floss_plot, phyl_beta_floss_plot, phyl_gamma_floss_plot, cols = 3)
```
<br>

#Discussion  
Increased human pressure did not appear to cause biotic homogenisation in our study site. Increased human footprint index (HFI), which measured the impact of increased population density, land transformation, accessibility, and electrical power infrastructure,  did not cause biotic homogenisation in taxonomic, functional or phylogenetic diversity. Instead, increased human footprint appeared to caused functional heterogenisation by increasing the functional diversity between sites. Forest loss did not appear to cause biotic homogenisation in any of the response variables but it did have a consistently negative impact on local and regional functional and phylogenetic diversity.

The result that increased human footprint appeared to increase functional diversity between sites is at odds with multiple previous studies that have shown homogenising effects of activities such as farming, forest management, and urbanisation (McKinney 2005, Rodrigues et al. 2013). However, it is important to note that in addition to studies showing the homogenising impacts of several studies have not shown biotic homogenisation in response to increased human pressure (Lee-Cruz et al. 2013).One potential reason for this could be that increased human impact caused an increase in habitat diversity, which in turn increased diversity between sites. Although we took the diversity of land classes within each unit into account it is possible that increased human impact diversified habitats at finer scales. Another potential cause of increased beta diversity in response to increased human impact could be related to the way that beta diversity was calculated.  Beta diversity was calculated as gamma diversity (regional diversity of a unit) divided by mean alpha diversity (mean local diversity of individual BBA sites). Thus, the diversity between sites (beta diversity) could increase in response to an increase in total diveristy or a decrease in mean local diversity. Human footprint did not have a statistically significant impact on functional gamma or alpha diversity (see appendix) but the negative estimate for local diversity was an order of magnitude stronger than that for regional diversity, which could have caused the positive impact on between-site diversity.

In addition to human footprint, forest loss did not appear to cause biotic homogenisation in any of the responses studied here. However, forest loss had a much clearer impact on local and regional diversity than human footprint. Forest loss appeared to have a negative impact on functional and phylogenetic diversity at the local and regional scales, although the impact on regional functional diversity was trending. Since forest loss had a negative impact at both local and regional scales, the method of calculating between-site diversity most likely canceled the impact of forest loss on between-site diversity, meaning that no homogenising impact could be seen. This suggests that even though units lost local and regional diversity, BBA sites did not become morea alike.
...
Our results suggest that increased human footprint and forest loss are not a cause of biotic homogenisation in our study site. Although our study did not have a temporal dimension to it, the spatial replicates (units) that had clearly differing levels of human footprint and forest loss should have overcome this.



Gordon et al. (2009) showed that agricultural intensification in coffee plantations lead to biotic homogenisation in beetle assemblages.
La Sorte & Boecklen (2005) provided evidence that bird communities in several areas in the USA have shown biotic homogenisation due to the expansion of a limited number of native common species, although they were not able to directly attribute a cause to this.
Areas subjected to urbanisation have been shown to exhibit phylogenetic homogenisation through the loss of specialists (Sol et al. 2017).
