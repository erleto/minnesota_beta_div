...
func_alpha_forest_change <- round(func_alpha_model_coef['Forest change', 'Estimate'] * forest_range, digits = 2)
...
func_gamma_forest_change <- round(func_gamma_model_coef['Forest change', 'Estimate'] * forest_range, digits = 2)
...
phyl_beta_npp_change <- round((phyl_beta_npp_perc / 100) * phyl_beta_range, digits = 2)

phyl_beta_temp_perc <- 100 - round(exp(phyl_beta_model_coef['Mean temperature', 'Estimate'] * temp_range) * 100, digits = 2)
phyl_beta_temp_change <- round((phyl_beta_temp_perc / 100) * phyl_beta_range, digits = 2)
...
phyl_alpha_forest_change <- round(phyl_alpha_model_coef['Forest change', 'Estimate'] * forest_range, digits = 2)
phyl_alpha_forest_perc <- round((phyl_alpha_forest_change / phyl_alpha_range) * 100, digits = 2)

phyl_gamma_forest_change <- round(phyl_gamma_model_coef['Forest change', 'Estimate'] * forest_range, digits = 2)
...
showed that plant species diversity has not significantly changed through time,
...
diversity is not necessarily the only goal [@SocolarHowShouldBetaDiversity2016].
For example, uneven losses in species between sites can lead to decreased mean
alpha diversity, which will increase the beta diversity of a region. On the
other hand, the colonisation of specialists into new sites can increase mean
alpha diversity [@McCuneGainsnativespecies2013], leading to a decrease in the
beta diversity of a region.  
...
causing the introduction of exotic and invasive species and extinctions of
native species have the potential to cause species composition in different
sites to become more similar [@BaiserPatternprocessbiotic2012]. Local species
extinctions often involve specialists with narrow dietary and habitat
preferences, and they are often replaced by generalists that can withstand
fluctuating environmental conditions and human-built environments
...
transport infrastructure, and forest change, causes sites to become more similar
to each other (decreased beta diversity). We also controlled for factors known
to impact biodiversity, including habitat diversity, net primary productivity,
and temperature [@QianEnvironmentrichnessrelationships2010]. We studied this
...
same spatial scale (see below). For those MNBBA points that had been counted
more than once we selected the earlier observation. We used MNBBA points located
at least `r 2 * count_radius`m from each other (twice the distance of the bird
count radius) to avoid overlap between bird counts. From pairs of MNBBA
points located under `r 2 * count_radius`m from each other we selected the
earlier observation, and if both points had the same date one was chosen
randomly. For the analyses in this study we used MNBBA points located in
forested land cover classes. To ensure that the dominant habitat being
sampled was forest, we first calculated the proportion of landcover types within
`r count_radius`m of each MNBBA count location using the ‘isectpolyrst’ tool in
Geospatial Modelling Environment version 0.7.3.0
...
community diversities, we only used units that had at least `r min_points` MNBBA
points located in a forested land use class (`r bba_forest_point_count` forested
mnbba points in `r forest_unit_count` units). We determined 3 points to be the
minimum number of communities to reliably estimate beta diversity.  
...
total removal of tree canopy cover within each pixel. Using this data we were
...
significantly impacted by three large forest fires (Pagami Creek (2011),
Cavity Lake (2006) and Ham Lake (2007) fires). Since the results from models
run with and without the affected units did not change significantly, we
decided to keep these units in the analyses.  
...
for functional and phylogenetic diversity were right skewed and were thus
...
(`r tabRefApp('tax_alpha_model_summary')`), since beta diversity would be 
expected to rise with increasing gamma diversity if alpha diversity does not 
change (like in the case of taxonomic alpha diversity, 
`r tabRefApp('tax_gamma_model_summary')`) or decreases.
...
functional beta diversity. Net primary (NPP) had a strong negative impact on functional beta diversity. 
...
but not impact on gamma diversity, which might be expected to decrease beta
...
As was the case with functional diversity, forest loss had a negative 
...
(-`r phyl_beta_temp_perc`%). NPP was not associated with either phlogenetic 
alpha or gamma diversity, although its association with . However, mean temperature had a positive impact on 
...
between sites. Although we considered the diversity of land classes within each
unit, it is possible that increased human impact diversified habitats at finer
scales. Another potential reason that beta diversity increases in response to
...
functional gamma or alpha diversity (see appendix 1 c & d) but the negative
estimate for local diversity was more than an order of magnitude stronger than that for
regional diversity, which may have been related to the positive impact on
between-site diversity. The negative estimate for the impact of human footprint
on functional alpha diversity suggests that human footprint lead to a loss of
functionally distinct species. If this loss occurred randomly, impacting some
areas but not others, this could lead to MNBBA count locations being more
different from each other. This loss of species would indicate that the
nestedness component of beta diversity was impacted, although impacts on
turnover cannot be ruled out [@BaselgaPartitioningturnovernestedness2010].
...
From a conservation viewpoint, the maximising of beta diversity, increasing
differences in diversity between sites, might be desireable. Ensuring that 
different areas host different species could safeguard against regional 
extinctions by providing a number of sources from where e.g. specialists 
could recolonise suitable habitats. Increasing between-site diversity could be
achieved by increasing total diversity. However, it would be difficult to argue
for increasing between-site diversity at the expense of decreasing local
diversity [@SocolarHowShouldBetaDiversity2016]. Indeed, our study appeared to
show this outcome. This result highlights the importance of critically
considering the use of beta diversity in a conservation setting. Our study also
highlights the importance of taking multiple facets of biodiversity into
account. The positive influence of increase human footprint on biodiversity in
our study region would have been missed if we had only focussed on taxonomic
diversity, as has often previously been the case. Similarly, impacts on
between-site and regional diversity would have been missed if we had only
studied local diversity.

# Conclusion  
Our results suggest that increased human footprint and forest change are not a
cause of biotic homogenisation in our study site.    
...
## 6. Spatial distribution of response variables and species richness
...
## 7. Spatial distribution of explanatory variables  
### a) Human footprint  
...
## 8. List of used R packages and their versions  
