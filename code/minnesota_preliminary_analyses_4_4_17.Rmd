...
since we would loose a whole bunch of data. Let's continue by inspecting nearest 
neghbour distances for points with unique coordinates.
...
but it does not show any clear distance within which BBA points are similar.
Since we are using bird counts within a 50m radius we will subset the points so 
we only include points that are at least 100m away from each other. We will 
include the earlier observation from each point pair, and randomy select a point 
...
units_points <- over(mnn_bba_points_unique_200m, units_merged)
...
beta_unit_list <- vector("list", length(unique(bird_data_whole$unit)))

for (unit in as.vector(unique(bird_data_whole$unit[bird_data_whole$unit == 515]))) {
  print(unit)
  ...
  bird_unit_wide <- bird_unit_wide[rowSums(bird_unit_wide) > 0, ]
  ...
  bird_div <- Rao(sample = bird_unit_wide, dfunc = funct_matrix, 
    dphyl = phylo_matrix, weight = FALSE, Jost = TRUE, structure = NULL)
  #beta_unit_list[[]] <- bird_div$TD$Beta_add
}
```