...
moran.test(units_merged_beta_subset$tax_div_resid, listw = units_merged_subset_lw, 
  alternative = 'two.sided', zero.policy = TRUE)
moran.mc(units_merged_beta_subset$tax_div_resid, units_merged_subset_lw, 999, 
  zero.policy = TRUE)
...
moran.test(units_merged_beta_subset$func_div_resid, listw = units_merged_subset_lw, 
  alternative = 'two.sided', zero.policy = TRUE)
moran.mc(units_merged_beta_subset$func_div_resid, units_merged_subset_lw, 999, 
  zero.policy = TRUE)
...
#phyl_model_lm <- lm(f_phyl_model, data = phyl_model_variables)
...
lm.morantest(phyl_model, listw = units_merged_subset_lw, alternative = 'two.sided', zero.policy = TRUE)

phyl_model_lag = spdep::lagsarlm(f_phyl_model, data = phyl_model_variables, listw = units_merged_subset_lw, tol.solve = 1.0e-30, zero.policy = TRUE)
#summary(phyl_model_lag)
#units_merged_beta_subset$phyl_div_resid <- residuals(phyl_model_lag)

phyl_model_error = spdep::errorsarlm(f_phyl_model, data = phyl_model_variables, listw = units_merged_subset_lw, tol.solve = 1.0e-30, zero.policy = TRUE)
#summary(phyl_model_error)
#units_merged_beta_subset$phyl_div_resid <- residuals(phyl_model_error)
AIC(phyl_model, phyl_model_lag, phyl_model_error)
...
phyl_model_lag <- spdep::lagsarlm(f_phyl_model, data = phyl_model_variables, listw = units_merged_subset_lw, 
  type = 'lag', method = 'MC', tol.solve = 1.0e-30, zero.policy = TRUE)
summary(phyl_model_lag, Nagelkerke = TRUE)

phyl_model_err <- spdep::errorsarlm(f_phyl_model, data = phyl_model_variables, listw = units_merged_subset_lw, 
  etype = 'error', method = 'MC', tol.solve = 1.0e-30, zero.policy = TRUE)
summary(phyl_model_err, Nagelkerke = TRUE)

phyl_model_durb <- spdep::lagsarlm(f_phyl_model, data = phyl_model_variables, listw = units_merged_subset_lw, 
  type = 'mixed', method = 'MC', tol.solve = 1.0e-30, zero.policy = TRUE)
summary(phyl_model_durb, Nagelkerke = TRUE)

phyl_model_errdurb <- spdep::errorsarlm(f_phyl_model, data = phyl_model_variables, listw = units_merged_subset_lw, 
  etype = 'emixed', method = 'MC', tol.solve = 1.0e-30, zero.policy = TRUE)
summary(phyl_model_errdurb, Nagelkerke = TRUE)

phyl_model_sac <- spdep::sacsarlm(f_phyl_model, data = phyl_model_variables, listw = units_merged_subset_lw, 
  type = 'sac', method = 'MC', tol.solve = 1.0e-30, zero.policy = TRUE)
summary(phyl_model_sac, Nagelkerke = TRUE)
...
summary(phyl_model_sma, Nagelkerke = TRUE)

AICs <- c(AIC(phyl_model_lm), AIC(phyl_model_lag), AIC(phyl_model_err), AIC(phyl_model_durb), 
...
W <- as(units_merged_subset_lw, "CsparseMatrix")
trMC <- trW(W, type = 'MC')
im <- impacts(phyl_model_lag, tr = trMC, R = 100)
...
moran.test(units_merged_beta_subset$phyl_div_resid, listw = units_merged_subset_lw, 
  alternative = 'two.sided', zero.policy = TRUE)
moran.mc(units_merged_beta_subset$phyl_div_resid, units_merged_subset_lw, 999, 
  zero.policy = TRUE)
...
We have clear spatial autocorrelation and a comparisson of AIC values shows that 
the best- fitting spatial structure is spherical. However, the problem is that 
we need to fit a GLM and also include spatial structure in the covariance 
structure. This is only possible in spgwr::ggwr, but the function is still 
preliminary and does not produce standard errors or p- values. Therefore, we 
compare the residuals of the original GLM, a ggwr model and a linear model with 
log- transformed phylogenetic diversity and a spatial covariance structure.
```{r, eval = FALSE}
#b1 <- gls(f_phyl_model, correlation = NULL, method = 'REML', data = phyl_model_variables)
#library(spgwr)
phyl_model_spher <- gls(f_phyl_model, correlation = corSpher(form = ~ x + y, nugget = TRUE), method = 'REML', data = phyl_model_variables)
#plot(Variogram(phyl_model_spher, maxDist = 250000))
units_merged_beta_subset$phyl_div_resid <- phyl_model_spher$residuals

phyl_model_ratio <- gls(f_phyl_model, correlation = corRatio(form = ~ x + y, nugget = TRUE), method = 'REML', data = phyl_model_variables)
#plot(Variogram(phyl_model_ratio, maxDist = 250000))
units_merged_beta_subset$phyl_div_resid <- phyl_model_ratio$residuals

phyl_model_exp <- gls(f_phyl_model, correlation = corExp(form = ~ x + y, nugget = TRUE), method = 'REML', data = phyl_model_variables)
#plot(Variogram(phyl_model_exp, maxDist = 250000))
units_merged_beta_subset$phyl_div_resid <- phyl_model_exp$residuals

phyl_model_gaus <- gls(f_phyl_model, correlation = corGaus(form = ~ x + y, nugget = TRUE), method = 'REML', data = phyl_model_variables)
#plot(Variogram(phyl_model_gaus, maxDist = 250000))
units_merged_beta_subset$phyl_div_resid <- phyl_model_gaus$residuals

anova(phyl_model_spher, phyl_model_ratio, phyl_model_exp, phyl_model_gaus, test = FALSE)

#aaa <- merge(units_merged_beta_subset[c('unit', 'phyl_beta_div')], explanatory_variables)
#f_phyl_model <- formula(phyl_beta_div ~ 
#    human_footprint + 
#    forest_loss + 
#    forest_point_freq + 
#    net_prim_prod + 
#    habitat_div)
#phyl_model_bw <- spgwr::ggwr.sel(f_phyl_model, data = aaa, family = Gamma(link = "inverse"), gweight = gwr.Gauss, #verbose = TRUE)
#phyl_model_ggwr <- spgwr::ggwr(f_phyl_model, data = aaa, family = Gamma(link = "inverse"), bandwidth = phyl_model_bw, #gweight = gwr.Gauss)
#units_merged_beta_subset$phyl_div_resid <- phyl_model_ggwr$lm$residuals
#phyl_model_gwr <- spgwr::gwr()
