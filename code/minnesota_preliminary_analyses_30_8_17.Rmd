...
# Panel functions for pairs plot
...
#npp <- read.table('./data/npp/mean_npp_unit_18_5_17.csv', sep = ',', dec = '.', header = TRUE)
npp <- read.table('./data/npp/mean_npp_unit_16_2_18.csv', sep = ';', dec = '.', header = TRUE)
npp <- npp[order(npp$unit), ]
# Scale values so that they are in kg/m^2/year
units_merged$net_prim_prod <- npp$npp * 0.0001
```
...
```
```{r, echo = FALSE, results = 'hold', message = FALSE, eval = FALSE}
# Join human footprint data to units_merged
...
multiplot(plot_variable_map(units_merged_fort, 'human_footprint'), 
  ...
  cols = 2)
```
Originally we dropped the four units clearly impacted by forest fires. However, 
after discussing this with Jerry, we decided to keep the units since dropping 
them would be a very articifical decision.
```{r, echo = FALSE, results = 'hold', message = FALSE}
units_merged_beta_subset <- units_merged_beta[!(units_merged_beta$unit %in% 
    c(39, 12, 38, 559)), ]
units_merged_beta_subset <- units_merged_beta
...
```{r save figs, results = 'hold', fig.show = 'hold', message = FALSE, warning = FALSE}
fig_height <- 19
fig_width <- 19
fig_dpi <- 600

png(filename = './figures/beta_div_plots.png', width = fig_width, 
    height = fig_height, units = 'cm', res = fig_dpi)
pairs(units_merged_beta_subset@data[c('tax_beta_div', 'func_beta_div', 
            'phyl_beta_div', 'forest_spp_richness')], 
      upper.panel = panel.cor.pearson, 
      lower.panel = panel.scatter, 
      diag.panel = panel.hist, 
      labels = c('Taxonomic \nbeta diversity', 'Functional \nbeta diversity', 
                 'Phylogenetic \nbeta diversity', 'Forest species \nrichness'), 
      cex.labels = 1.3)
dev.off()

png(filename = './figures/human_impact_plots.png', width = fig_width, 
    height = fig_height, units = 'cm', res = fig_dpi)
pairs(units_merged_human_impact_subset, 
      upper.panel = panel.cor.spearman, 
      lower.panel = panel.scatter, 
      diag.panel = panel.hist, 
      labels = c('Human \nfootprint', 'Forest change', 'Total \npopultation', 
                 'Road density', 'Human \nland use'), 
      cex.labels = 1.3)
dev.off()

png(filename = './figures/explanatory_plots.png', width = fig_width, 
    height = fig_height, units = 'cm', res = fig_dpi)
pairs(units_merged_beta_subset@data[c('human_footprint', 'forest_loss', 
      'habitat_div', 'net_prim_prod', 'mean_temp', 'forest_point_freq')], 
      upper.panel = panel.cor.spearman, 
      lower.panel = panel.scatter, 
      diag.panel = panel.hist, 
      labels = c('Human \nfootprint', 'Forest change', 'Habitat \ndiversity', 
                 'Net primary \nproductivity', 'Mean \ntemperature', 'Forest point \nfrequency'), 
      cex.labels = 1.3)
dev.off()

for_point_hist <- ggplot(units_merged_beta_subset@data, aes(x = forest_point_freq)) + 
  ...
  mytheme
ggsave('for_point_hist.png', plot = for_point_hist, device = 'png', path = './figures/',
  scale = 1, width = fig_width*(2/3), height = fig_height/2, units = 'cm', dpi = fig_dpi)
```

```{r save comparison histogram, echo = FALSE, results = 'hold', message = FALSE}
units_merged$forest_point_class <- plyr::mapvalues(units_merged$forest_point_freq, 
                                      from = c(0:18), to = c(0, 0, 0, rep(1, 16)))

units_merged$forest_point_class <- plyr::mapvalues(units_merged$forest_point_freq, 
    from = c(0:18), to = c(rep('Units not used in study', 3), rep('Units used in study', 16)))

hfi_boxplot <- ggplot(units_merged@data, aes(forest_point_class, human_footprint)) + 
  geom_boxplot(colour = 'black') + 
  xlab('') + 
  ylab('Human footprint index') + 
  scale_y_continuous(limits = c(0, 80), expand = c(0, 0)) + 
  mytheme
ggsave('hfi_boxplot.png', plot = hfi_boxplot, device = 'png', path = './figures/',
  scale = 1, width = fig_width, height = fig_height / 2, units = 'cm', dpi = fig_dpi, limitsize = FALSE)
        
units_merged_subset_hfi_mean <- 
  base::mean(units_merged$human_footprint[units_merged$forest_point_class == 1])
units_merged_subset_hfi_median <- 
  stats::median(units_merged$human_footprint[units_merged$forest_point_class == 1])
units_merged_subset_hfi_min <- 
  base::min(units_merged$human_footprint[units_merged$forest_point_class == 1])
units_merged_subset_hfi_max <- 
  base::max(units_merged$human_footprint[units_merged$forest_point_class == 1])

units_merged_subset_inv_hfi_mean <- 
  base::mean(units_merged$human_footprint[units_merged$forest_point_class == 0])
units_merged_subset_inv_hfi_median <- 
  stats::median(units_merged$human_footprint[units_merged$forest_point_class == 0])
units_merged_subset_inv_hfi_min <- 
  base::min(units_merged$human_footprint[units_merged$forest_point_class == 0])
units_merged_subset_inv_hfi_max <- 
  base::max(units_merged$human_footprint[units_merged$forest_point_class == 0])
```


## Create and save maps
```{r save maps, results = 'hold', fig.show = 'hold', message = FALSE, warning = FALSE}
tax_beta_map <- ggplot(data = units_merged_beta_subset_fort, 
  ...
  geom_polygon(data = minnesota, aes(long, lat, group = group), fill = NA, color = 'black') + 
  coord_equal() + theme_void()
ggsave('tax_beta_map.png', plot = tax_beta_map, device = 'png', path = './figures/',
  scale = 1, width = fig_width, height = fig_height, units = 'cm', dpi = fig_dpi)

func_beta_map <- ggplot(data = units_merged_beta_subset_fort, 
  ...
  geom_polygon(data = minnesota, aes(long, lat, group = group), fill = NA, color = 'black') + 
  coord_equal() + theme_void()
ggsave('func_beta_map.png', plot = func_beta_map, device = 'png', path = './figures/',
  scale = 1, width = fig_width, height = fig_height, units = 'cm', dpi = fig_dpi)

phyl_beta_map <- ggplot(data = units_merged_beta_subset_fort, 
  ...
  geom_polygon(data = minnesota, aes(long, lat, group = group), fill = NA, color = 'black') + 
  coord_equal() + theme_void()
ggsave('phyl_beta_map.png', plot = phyl_beta_map, device = 'png', path = './figures/',
  scale = 1, width = fig_width, height = fig_height, units = 'cm', dpi = fig_dpi)

spp_rich_map <- ggplot(data = units_merged_beta_subset_fort, #units_merged_beta_subset_fort
  ...
  geom_polygon(data = minnesota, aes(long, lat, group = group), fill = NA, color = 'black') + 
  coord_equal() + theme_void()
ggsave('spp_rich_map.png', plot = spp_rich_map, device = 'png', path = './figures/',
  scale = 1, width = fig_width, height = fig_height, units = 'cm', dpi = fig_dpi)

hfp_map <- ggplot(data = units_merged_fort, 
  ...
  geom_polygon(data = minnesota, aes(long, lat, group = group), fill = NA, color = 'black') + 
  coord_equal() + theme_void()
ggsave('hfp_map.png', plot = hfp_map, device = 'png', path = './figures/',
  scale = 1, width = fig_width, height = fig_height, units = 'cm', dpi = fig_dpi)

f_loss_map <- ggplot(data = units_merged_fort, #units_merged_beta_subset_fort
  ...
  geom_polygon(data = minnesota, aes(long, lat, group = group), fill = NA, color = 'black') + 
  coord_equal() + theme_void()
ggsave('f_loss_map.png', plot = f_loss_map, device = 'png', path = './figures/',
  scale = 1, width = fig_width, height = fig_height, units = 'cm', dpi = fig_dpi)

hab_div_map <- ggplot(data = units_merged_fort, 
  ...
  geom_polygon(data = minnesota, aes(long, lat, group = group), fill = NA, color = 'black') + 
  coord_equal() + theme_void()
ggsave('hab_div_map.png', plot = hab_div_map, device = 'png', path = './figures/',
  scale = 1, width = fig_width, height = fig_height, units = 'cm', dpi = fig_dpi)

npp_map <- ggplot(data = units_merged_fort, 
  aes_string(x = 'long', y = 'lat', fill = 'net_prim_prod', group = 'group')) + 
  geom_polygon(data = minnesota, aes(long, lat, group = group), fill = 'lightgray', color = NA) + 
  geom_polygon(color = 'black', size = .05) + 
  scale_fill_gradientn(colours = rev(terrain.colors(10)), 
      name = expression(atop("Net primary\nproductivity", 
                             paste('(C kg/m'^{2}, '/year)')))) + 
  geom_polygon(data = minnesota, aes(long, lat, group = group), fill = NA, color = 'black') + 
  coord_equal() + theme_void() 
ggsave('npp_map.png', plot = npp_map, device = 'png', path = './figures/',
  scale = 1, width = fig_width, height = fig_height, units = 'cm', dpi = fig_dpi)

mean_temp_map <- ggplot(data = units_merged_fort, 
  ...
  geom_polygon(data = minnesota, aes(long, lat, group = group), fill = NA, color = 'black') + 
  coord_equal() + theme_void()
ggsave('mean_temp_map.png', plot = mean_temp_map, device = 'png', path = './figures/',
  scale = 1, width = fig_width, height = fig_height, units = 'cm', dpi = fig_dpi)
```
  
# Relevant links
[Modeling skewed continuous outcome using Gamma family in glm()](https://rstudio-pubs-static.s3.amazonaws.com/5691_192685385fc445c9b3fb1619960a20e2.html)  
[Analysing spatial data in R: Spatial autocorrelation](http://www.bias-project.org.uk/ASDARcourse/unit6_slides.pdf)  
[Spatial Regression Analysis in R](http://www.csiss.org/gispopsci/workshops/2011/PSU/readings/W15_Anselin2007.pdf)  
[Spatial regression models](http://rspatial.org/analysis/rst/7-spregression.html)  
[Spatially Autoregressive Models 1](https://rpubs.com/corey_sparks/246342)  
[Spatially Autoregressive Models 2](https://rpubs.com/corey_sparks/250314)  
[Exploratory Spatial Data Analysis](https://rpubs.com/corey_sparks/242698)
[Fitting Gamma GLMs in Multiple Ways](http://seananderson.ca/2014/04/08/gamma-glms.html)