...
whole_area_div_summary <- data.frame(
  ...
  beta_prop_norm = c(bird_div_all$TD$Beta_prop / (1 - (1 / ncol(bird_points_species_wide))), 
    bird_div_all$FD$Beta_prop / (1 - (1 / ncol(bird_points_species_wide))), 
    bird_div_all$PD$Beta_prop / (1 - (1 / ncol(bird_points_species_wide)))), 
...
```{r, results = 'hold', fig.show = 'hold', eval = FALSE}
# Test for spatial autocorrelation
moran.test(units_merged_beta_subset$tax_div_resid, listw = units_merged_subset_lw, 
  alternative = 'two.sided', zero.policy = TRUE)
moran.mc(units_merged_beta_subset$tax_div_resid, units_merged_subset_lw, 
...
phyl_model <- lm(f_phyl_model, data = phyl_model_variables)
phyl_model_spatial <- spdep::errorsarlm(f_phyl_model, data = phyl_model_variables, 
