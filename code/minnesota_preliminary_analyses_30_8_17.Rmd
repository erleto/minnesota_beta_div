...
units_road_density$Dens_Minor <- as.numeric(units_road_density$Dens_Minor)
...
```{r}
units_merged_human_impact <- dplyr::select(units_merged, human_footprint, 
  human_pca, 'forest_loss' = allV1, 'total_pop' = censusSUM, 
  'major_highways' = Dens_Major, 'minor_highways' = Dens_Minor, 
  'other_roads' = Dens_Other, 'quarries' = HabV15, 
  'low_intensity_development' = HabV16, 'med_intensity_development' = HabV17, 
  'high_intensity_development' = HabV18, 'croplands' = HabV26)
...
for (unit in as.vector(common_units$unit)) {
  ...
  func_gamma_unit_list[[counter]] <- bird_div$FD$Gamma
  phyl_gamma_unit_list[[counter]] <- bird_div$PD$Gamma
  #########
}
...
pairs(units_merged_human_impact, cex.labels = 1.2, font.labels = 1.6, diag.panel = panel.hist, upper.panel = panel.cor)
```
...
#Analyse taxonomic beta diversity
...
tax_model <- lm(f_tax_model, data = tax_model_variables)
tax_model_err <- spdep::errorsarlm(f_tax_model, data = tax_model_variables, listw = units_merged_subset_lw, 
  etype = 'error', tol.solve = 1.0e-30, zero.policy = TRUE)
tax_model_spatial <- tax_model_err

units_merged_beta_subset$tax_div_resid <- residuals(tax_model_spatial)
tax_model_summary <- summary(tax_model_spatial)
...
```{r}
# Get standardised predicted and residual values
tax_standardised_pred <- (predict(tax_model_spatial) - mean(predict(tax_model_spatial))) / sd(predict(tax_model_spatial))
tax_standardised_resid <- (resid(tax_model_spatial) - mean(resid(tax_model_spatial))) / sd(resid(tax_model_spatial))
# Create standardised residuals plot
plot(tax_standardised_pred, tax_standardised_resid, main = "Standardised 
  taxonomic residuals plot", xlab = "Standardised predicted values", ylab = "Standardised residuals")
abline(0,0)

# Histogram of standardised residuals
hist(tax_standardised_resid, freq = FALSE, main = 'Histogram of taxonomic 
  model residuals', xlab = 'Residuals')
curve(dnorm, add = TRUE)

# Probability- probability plot of standardised residuals
tax_prob_dist <- pnorm(tax_standardised_resid)
plot(ppoints(length(tax_standardised_resid)), sort(tax_prob_dist), main = "PP Plot", 
...
max(tax_model_variables$net_prim_prod) - min(tax_model_variables$net_prim_prod)
```
...
#Analyse functional beta diversity
```{r, results = 'hold', message = FALSE}
...
#Analyse phylogenetic beta diversity
...
~~Let's print a map to show local differences in residual values.~~
```{r, message = FALSE, warning = FALSE, fig.width = 8, fig.height = 4, echo = FALSE, eval = FALSE}
# Plot residual values on map
units_merged_beta_subset$id <- rownames(units_merged_beta_subset@data)
units_merged_beta_subset_fort <- fortify(units_merged_beta_subset, region = 'unit')
units_merged_beta_subset_fort <- left_join(units_merged_beta_subset_fort, 
