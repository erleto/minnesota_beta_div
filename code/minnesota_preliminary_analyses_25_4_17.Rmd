...
moran_plot_func <- function(a, b) {
  df <- a
  response_var <- df@data[, deparse(substitute(b))]
...
# neighbours.
prepare_moran_data <- function(a, b) {
  df <- a
  response_var <- df@data[, deparse(substitute(b))]
  spatmatrix <- spdep::poly2nb(df)
  spat_listw <- spdep::nb2listw(spatmatrix, zero.policy = TRUE)
...
moran_mc_func(units_merged_beta, beta_div, 999)
...
beta_div_map <- ggplot(data = df, aes(x = long, y = lat, fill = beta_div, group = group)) + 
  geom_polygon(color = "white", size = .05) + 
  scale_fill_gradientn(colours = rev(terrain.colors(10)), name = 'Beta diversity') + 
  geom_polygon(data = minnesota, aes(long, lat, group = group), fill = NA, color = "black") + 
  coord_equal() + theme_void()

# Plot map of local spatial autocorrelation
moran_data <- prepare_moran_data(units_merged_beta, beta_div)
...
change_multiplot <- multiplot(beta_div_map, lisa_map, cols = 2)
```

Beta diversity values appear to be spatially autocorrelated, with a group of 
high values in the northeast of the state.
...
model_variables <- dplyr::inner_join(units_merged_beta_subset@data[c('unit', 'beta_div')], 
  dplyr::select(explanatory_variables, -human_pca))
m <- lm(beta_div ~ human_footprint + forest_loss + other_roads + mean_temperature + 
...
# Calculate global Moran's I with permutation test (function)
moran_mc_func(units_merged_beta_subset, resid, 999)

plot(variogram(resid ~ 1, locations = coordinates(units_merged_beta_subset), 
...
moran_plot_func(units_merged_beta_subset, resid)
```
...
However, the Moran's I test does not show statistically significant spatial 
autocorrelation, which is confirmed by the Moran's scatter plot.
...
resid_map <- ggplot(data = df, aes(x = long, y = lat, fill = resid, group = group)) + 
  geom_polygon(color = "white", size = .05) + 
...
# Plot map of local spatial autocorrelation
moran_data <- prepare_moran_data(units_merged_beta_subset, resid)
lisa_map <- moran_data %>% ggplot(aes(long, lat, group = group, fill = quad_sig)) + 
  geom_polygon(color = "white", size = .05) + 
  scale_fill_brewer(palette = "Set1", name = "Local Moran's I", 
    breaks=c("high-high", "low-low", "non_sign"), 
