...
* [Check model residuals](#check_model_res)  
...
# Prepare data for plotting local Moran's I. Requires 
# SpatialPolygonsDataFrame, column name, and number of 
# neighbours.
prepare_moran_data <- function(df, col_name, neighbours) {
  response_var <- df@data[, deparse(substitute(col_name))]
  spat_nb <- nblag(spdep::poly2nb(df, row.names = df$unit), maxlag = 5)
  spat_listw <- spdep::nb2listw(spat_nb[[neighbours]], zero.policy = TRUE)
  ...
  df_fort <- fortify(df, region = "id")
  left_join(df_fort, df@data)
}
...
Before continuing any further let's define the radius for bird counts and 
...
components), so we will it to our dataset as an estimation of human impact. 
...
Then we clean and join the functional data before calculating functional 
distances between units. The functional data is from the paper od Belmaker et 
...
bird_func_data_subset <- dplyr::left_join(bird_names, bird_func_data, 
  by = c("common" = "English"))
# Species that have their rows cut short
#bird_func_data_subset$common[is.na(bird_func_data_subset$Nocturnal)]
#dplyr::filter(bird_func_data_subset, grepl('Blue-headed Vireo', common))
```
...
trait_distance <- as.matrix(daisy(trait_matrix, metric = "gower"))
...
explanatory_variables <- dplyr::select(units_merged_beta@data, unit, 
  ...
  human_pca, 
  'forest_loss' = allV1, 
  'other_roads' = Dens_Other, 
  'mean_temperature' = tempMN, 
  'mean_precipitation' = precMN, 
  'forest_point_freq' = forest_point_freq)
```
...
ggplot(explanatory_variables_long, aes(factor(0),value)) +
  geom_boxplot() +  facet_wrap(~key, scales = "free", ncol = 7) + 
...
```{r}
hist(units_merged_beta$tax_beta_div, main = 'Distribution of beta values', xlab = 'Mean beta per unit')
```
...
# Plot beta diversity values on map
...
tax_beta_div_map <- ggplot(data = df, aes(x = long, y = lat, fill = tax_beta_div, group = group)) + 
  geom_polygon(color = "white", size = .05) + 
  scale_fill_gradientn(colours = rev(terrain.colors(10)), name = 'Taxonomic beta diversity') + 
  geom_polygon(data = minnesota, aes(long, lat, group = group), fill = NA, color = "black") + 
  coord_equal() + theme_void()

# Plot map of local spatial autocorrelation
moran_data <- prepare_moran_data(units_merged_beta, tax_beta_div, 1)
lisa_map <- moran_data %>% ggplot(aes(long, lat, group = group, fill = quad_sig)) + 
  geom_polygon(color = "white", size = .05) + 
...
change_multiplot <- multiplot(tax_beta_div_map, lisa_map, cols = 2)
```
...
model_variables <- dplyr::inner_join(units_merged_beta_subset@data[c('unit', 
  'tax_beta_div')], dplyr::select(explanatory_variables, -human_pca))
m <- lm(tax_beta_div ~ human_footprint + forest_loss + other_roads + mean_temperature + 
    mean_precipitation + forest_point_freq, data = model_variables) #other_roads + 
summary(m)
units_merged_beta_subset$tax_div_resid <- m$residuals
car::vif(m)
```
...
df <- fortify(units_merged_beta_subset, region = 'unit')
df <- left_join(df, units_merged_beta_subset@data)
resid_map <- ggplot(data = df, aes(x = long, y = lat, fill = tax_div_resid, group = group)) + 
  geom_polygon(color = "white", size = .05) + 
  scale_fill_gradientn(colours = rev(terrain.colors(10)), name = 'Model residuals') + 
  geom_polygon(data = minnesota, aes(long, lat, group = group), fill = NA, color = "black") + 
  coord_equal() + theme_void()

# Plot map of local spatial autocorrelation
moran_data <- prepare_moran_data(units_merged_beta_subset, tax_div_resid, 1)
lisa_map <- moran_data %>% ggplot(aes(long, lat, group = group, fill = quad_sig)) + 
  geom_polygon(color = "white", size = .05) + 
  scale_fill_brewer(palette = "Set1", name = "Local Moran's I", 
    breaks=c("high-high", "low-low", "non_sign"), 
...
change_multiplot <- multiplot(resid_map, lisa_map, cols = 2)
```
