...
units_beta <- data.frame(unit = common_units$unit, tax_beta_div = unlist(tax_beta_unit_list), 
  ...
  spp_richness = unlist(spp_unit_list))
...
units_merged_beta_subset <- units_merged_beta[!(units_merged_beta$unit %in% 
    c(39, 12, 38, 559)), ]
units_merged_nb <- poly2nb(units_merged_beta_subset)
...
moran.test(units_merged_beta_subset$tax_div_resid, listw = units_merged_lw, 
  alternative = 'two.sided', zero.policy = TRUE)
moran.mc(units_merged_beta_subset$tax_div_resid, units_merged_lw, 999, 
  zero.policy = TRUE)
...
```{r, results = 'hold', fig.show = 'hold'}
# Test for spatial autocorrelation
moran.test(units_merged_beta_subset$func_div_resid, listw = units_merged_lw, 
  alternative = 'two.sided', zero.policy = TRUE)
moran.mc(units_merged_beta_subset$func_div_resid, units_merged_lw, 999, 
  zero.policy = TRUE)
...
phyl_model = spdep::lagsarlm(f_phyl_model, data = phyl_model_variables, listw = units_merged_lw, tol.solve = 1.0e-30, zero.policy = TRUE)
#AIC(phyl_model)
phyl_model = spdep::errorsarlm(f_phyl_model, data = phyl_model_variables, listw = units_merged_lw, tol.solve = 1.0e-30, zero.policy = TRUE)
#AIC(phyl_model)
...
```{r, results = 'hold', fig.show = 'hold'}
# Test for spatial autocorrelation
moran.test(units_merged_beta_subset$phyl_div_resid, listw = units_merged_lw, 
  alternative = 'two.sided', zero.policy = TRUE)
moran.mc(units_merged_beta_subset$phyl_div_resid, units_merged_lw, 999, 
  zero.policy = TRUE)
...
```{r}
summary(units_merged_beta_subset@data[c('tax_beta_div', 'func_beta_div', 'phyl_beta_div', 'spp_richness')])
```
...to be continued...